...
uint8_t serial_slave_buffer[SERIAL_SLAVE_BUFFER_LENGTH];
uint8_t serial_master_buffer[SERIAL_MASTER_BUFFER_LENGTH];
uint8_t serial_slave_screen_buffer[SERIAL_SCREEN_BUFFER_LENGTH];
uint8_t slave_buffer_change_count;
bool hid_screen_change; // Flag marking if the screen data is dirty and needs transferring to slave
...
void matrix_render_user(struct CharacterMatrix *matrix) {
  if (is_keyboard_master()) {
    ...
    // matrix_write_P(matrix, write_layer());
